    ...
    if (isset($_SESSION["user_id"]) && ($_SESSION["user_id"] == $_POST["user_id"] || $_SESSION["role"] == 1)) {
        ...
        if( isset($_FILES["profile_pic"]) && $_FILES ["profile_pic"]["error"] == 0) {
            if( 
                ...
                &&
                $_FILES ["profile_pic"]["size"] < 20000000000
            ) {
                ...
                if( !file_exists( $file_name) ) {
                    ...
                    if (move_uploaded_file(
                        $_FILES["profile_pic"]["tmp_name"], $file_name )){
...
elseif( isset($_POST["action"]) && $_POST["action"] =="change_password") :
    //select current user, check password matches, get new passwords, check if they match, if true update password for user
    $user_id = $_POST["user_id"];
